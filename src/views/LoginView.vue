<template>
  <div id="visa">
    ...
    <form @submit.prevent="handleSubmission">
      ...
      <input type="text" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$" class="invalid:border-invalidForm  invalid:text-invalidForm 
       focus:invalid:border-invalidForm  focus:invalid:ring-invalidForm placeholder:italic border-bluPadelHub shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" v-model="email" required > <br>
      ...
      <input type="password" pattern=".{8,}" class="border-bluPadelHub invalid:border-invalidForm  invalid:text-invalidForm 
       focus:invalid:border-invalidForm  focus:invalid:ring-invalidForm shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline placeholder:italic" v-model="password" required><br>
      <span v-if="msg.password" class="italic text-xs">{{msg.password}}</span>
      <br>
      <br>
      <input class="shadow bg-bluPadelHub enabled:hover:bg-bluPadelHubHover text-white font-bold py-3 w-full rounded-[12px] disabled:opacity-50 disabled:cursor-not-allowed" type="submit" :disabled="!disabled.every((i:any) => i === false)" v-on:click.prevent = "login()"/>
    </form>
</div>
</template>
  ...
  methods:{
    ...
    validatePassword(value:any){
      if (value.length >= 7) {
        ...
        this.msg['password'] = 'Almeno 8 caratteri' ;
